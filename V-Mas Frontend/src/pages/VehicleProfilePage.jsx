...
export default function VehicleProfilePage() {
  ...
  return (
    <Shell>
      ...
      <div style={{ maxWidth: 860, margin: '0 auto', padding: '24px 24px 60px', animation: 'fadeUp 0.4s ease both' }}>

        {/* Back button */}
        <button
          onClick={() => navigate('/vehicles')}
          style={{ display: 'inline-flex', alignItems: 'center', gap: 8, marginBottom: 20, padding: '8px 18px', borderRadius: 10, border: `1px solid ${D.border}`, background: D.surface, color: D.textSub, cursor: 'pointer', fontWeight: 700, fontSize: '0.82rem', fontFamily: 'inherit', transition: 'all 0.2s' }}
          onMouseEnter={e => { e.currentTarget.style.color = D.text; e.currentTarget.style.borderColor = D.blue }}
        ...
        {/* Hero card */}
        <div style={{ borderRadius: 24, overflow: 'hidden', border: `1px solid ${D.border}`, boxShadow: isDark ? '0 8px 40px rgba(0,0,0,0.45)' : '0 8px 40px rgba(0,0,0,0.12)', marginBottom: 24 }}>
          <div style={{ position: 'relative', height: 260, background: isDark ? 'linear-gradient(160deg,rgba(30,41,59,0.95),rgba(15,23,42,0.9))' : 'linear-gradient(160deg,#f8faff,#eef2ff)', overflow: 'hidden' }}>
            {vehicle.vehicleImage
              ...
              : <div style={{ width: '100%', height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 12 }}>
                <Car size={72} style={{ color: isDark ? 'rgba(148,163,184,0.35)' : 'rgba(99,102,241,0.25)' }} />
                <span style={{ color: D.textFaint, fontSize: '0.82rem' }}>No photo available</span>
              </div>}
            <div style={{ position: 'absolute', top: 18, right: 18, background: sc.bg, color: sc.color, border: `1px solid ${sc.border}`, padding: '5px 14px', borderRadius: 99, fontSize: '0.75rem', fontWeight: 800, textTransform: 'uppercase', backdropFilter: 'blur(8px)' }}>{vehicle.status}</div>
          </div>
          <div style={{ background: D.surface, padding: '20px 32px', borderTop: `1px solid ${D.border}` }}>
            <h1 style={{ margin: 0, fontSize: '1.6rem', fontWeight: 900, color: D.text, letterSpacing: '-0.02em' }}>{vehicle.manufacturer} {vehicle.model}</h1>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10, marginTop: 10 }}>
              <span style={{ background: isDark ? 'rgba(99,102,241,0.15)' : 'rgba(99,102,241,0.08)', color: isDark ? '#818cf8' : '#4f46e5', border: `1px solid ${isDark ? 'rgba(99,102,241,0.3)' : 'rgba(99,102,241,0.2)'}`, padding: '4px 12px', borderRadius: 8, fontSize: '0.75rem', fontWeight: 800 }}>{vehicle.registrationNo}</span>
              {vehicle.year && <span style={{ color: D.textSub, fontSize: '0.85rem', fontWeight: 600, padding: '4px 0' }}>{vehicle.year}</span>}
              <span style={{ background: D.blueDim, color: D.blue, padding: '4px 12px', borderRadius: 99, fontSize: '0.72rem', fontWeight: 800, textTransform: 'uppercase' }}>{formatFuelType(vehicle.fuelType) || 'UNKNOWN'}</span>
            </div>
          </div>
        </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 20, animation: 'fadeUp 0.3s ease both' }}>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(180px,1fr))', gap: 12 }}>
              {[
                ...
                <div key={i} onClick={item.click} title={item.click ? 'Update Mileage' : ''}
                  ...
                  <span style={{ fontSize: '0.9rem', color: D.text, fontWeight: 700, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{item.value}</span>
                </div>
              ))}
            </div>

            {(isAdmin || isController) && (
              <div style={{ background: D.surface, borderRadius: 16, border: `1px solid ${D.border}`, padding: '18px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16, flexWrap: 'wrap' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                  <div style={{ width: 44, height: 44, borderRadius: 12, background: vehicle.driverUsername ? D.greenDim : D.surfaceHi, color: vehicle.driverUsername ? D.green : D.textFaint, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    {vehicle.driverUsername ? <UserCheck size={20} /> : <User size={20} />}
                  </div>
                  <div>
                    <div style={{ fontSize: '0.68rem', fontWeight: 800, color: D.textSub, textTransform: 'uppercase', marginBottom: 3 }}>Assigned Driver</div>
                    {vehicle.driverUsername
                      ? <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                        <span style={{ fontSize: '0.95rem', fontWeight: 800, color: D.text }}>{vehicle.driverUsername}</span>
                        <span style={{ fontSize: '0.65rem', fontWeight: 800, padding: '2px 8px', borderRadius: 6, background: D.greenDim, color: D.green }}>ASSIGNED</span>
                      </div>
            ...
            <div style={{ background: D.surface, border: `1px solid ${D.border}`, borderRadius: 16, padding: '20px 24px', display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              <div style={{ borderTop: `1px solid ${D.border}`, paddingTop: 16 }}>
                <h5 style={{ margin: '0 0 12px', fontSize: '0.75rem', fontWeight: 800, color: D.textSub, textTransform: 'uppercase', letterSpacing: '0.04em' }}>Documents & Papers</h5>
                  ...
                  return (
                    <div style={{ background: D.surface, border: `1px solid ${D.border}`, borderRadius: 16, padding: '20px 24px', display: 'flex', flexDirection: 'column', gap: 12 }}>
                      ...
                      <div style={{ display: 'flex', alignItems: 'baseline', gap: 6 }}>
                        <span style={{ fontSize: '2rem', fontWeight: 900, color: D.text }}>{s.fuelEfficiency?.toFixed(1) || '0.0'}</span>
                        <span style={{ fontSize: '0.95rem', color: D.textSub, fontWeight: 700 }}>km / Liter</span>
                      </div>
                      <div style={{ height: 7, background: 'rgba(255,255,255,0.06)', borderRadius: 999, overflow: 'hidden' }}>
                        <div style={{ width: `${Math.min((s.fuelEfficiency || 0) * 6.6, 100)}%`, height: '100%', background: ec, borderRadius: 999 }} />
                      </div>
